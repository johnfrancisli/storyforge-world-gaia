---
id: thread:valdris-succession
name: The Crown in Balance
thread_type: quest
stakes: If the king dies without naming an heir, civil war. If Roderick wins, the old order hardens and
  the reformists are purged. If Elara wins, the dragon order is reformed and the old nobility loses power.
  If Tomas wins — no one knows, including Tomas.
pressure: rising
participants:
- character:king-aldran
- character:princess-elara
- character:prince-roderick
locations:
- location:crownhaven
- location:valdris
organizations:
- org:dragon-order
known_clues: []
next_possible_moves:
- The king's physician quietly informs the court that the king's condition is worsening — the information
  leaks within a day.
- 'Roderick makes Garrick a formal offer: pardon and a commission in exchange for blocking the Greywood
  road against Elara''s merchant allies.'
- Elara's reformist researchers achieve a partial resonance from a dragonbone artifact — the first proof
  that an artificial bond might be possible — and someone in her team talks.
resolution: null
---

King Aldran is dying and has not named an heir. Princess Elara backs the reformist faction and secret dragonbone research. Prince Roderick commands the military and is secretly courting the bandit lord Garrick as a deniable asset. The youngest, Tomas, is unaligned. The court is a cold war of positioning, and the king is running out of time.
