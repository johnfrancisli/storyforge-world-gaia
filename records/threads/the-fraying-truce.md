---
id: thread:sangguo-truce
name: The Fraying Truce
thread_type: quest
stakes: If the truce breaks, the largest war in a generation begins. The last war killed tens of thousands.
  Sangguo's trade routes feed half of Gaia — a war disrupts everything, everywhere.
pressure: rising
participants:
- character:cao-shen
locations:
- location:sangguo
- location:jin-an
organizations:
- org:jin-state
- org:chu-state
- org:wei-state
known_clues: []
next_possible_moves:
- A second skirmish at the Long Pass — this time with casualties on both sides, each blaming the other.
- Cao Shen's court physician is found dead in the palace, an apparent suicide that the warlord's spymaster
  does not believe was suicide.
- Chu's warlord Sun Liang receives a secret envoy from Wei proposing an alliance against Jin — and the
  envoy is intercepted by Jin scouts.
resolution: null
---

The three-year truce between Jin, Chu, and Wei is nearing its end. One border skirmish has already happened between Jin and Wei forces at the Long Pass. The younger officers at the Great Academy in Jin'an speak openly about ending the truce. Warlord Cao Shen is dying faster than anyone knows and wants reunification before he goes.
