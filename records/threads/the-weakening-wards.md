---
id: thread:shrine-network
name: The Weakening Wards
thread_type: quest
stakes: If the ward chain breaks along the Cedar Road, the pilgrimage route becomes unsafe and the mountain
  villages lose their protection. Wild yokai are not monsters — they are unpredictable, and people die
  when the boundary is weak.
pressure: steady
participants:
- character:amemiya-sora
locations:
- location:tsukuyomi
- location:haru-shrine-town
organizations:
- org:ashikara-shogunate
known_clues: []
next_possible_moves:
- Sora notices the upstream shrine's ward has gone dark — the keeper there has left, and no replacement
  was sent.
- A farmer from the next valley arrives in Haru's Shrine Town reporting yokai sightings on the Cedar Road
  — something that has not happened in a generation.
- 'A messenger from the Great Shrine in Tsuki-no-Miyako arrives with an order: Sora is to report to the
  capital for reassignment. Her shrine will be ''temporarily'' unstaffed.'
resolution: null
---

In Tsukuyomi, the shrine network that keeps wild yokai at a distance from settled areas is weakening. Rural shrines are being neglected as the central authority funnels resources to the capital. Miko Sora maintains her shrine alone, but the ward she tends is one of a chain, and links upstream of her are failing. If enough links break, the yokai pressure on the Cedar Road becomes dangerous.
