---
id: relationship:quilla-and-tito
name: Master feather worker and headdress restorer
participants:
- character:quilla
- character:tito
association: mentor-student
public_status: Tito works under Quilla, learning the restoration of old headdresses
  and ceremonial featherwork.
bond: Quilla is passing on a craft that nearly died with her generation, and Tito is the only apprentice
  with the patience for it. What Tito does not yet know is that Quilla is also passing on the meanings
  encoded in the patterns — a ceremonial language the tribe believes is lost. She is deciding, lesson
  by lesson, whether he is ready to be told.
disposition: 73
summary: Quilla is passing on a craft that nearly died with her generation, and Tito is the only apprentice
  with the patience for it. What Tito does not yet know is that Quilla is also passing on the meanings
  encoded in the patterns — a ceremonial language the tribe believes is lost. She is deciding, lesson
  by lesson, whether he is ready to be told.
current_stage: demanding_apprenticeship
perspectives:
- subject: character:quilla
  object: character:tito
  disposition: 78
  summary: Quilla believes Tito is worth the severity of the work and feels responsible for what the teaching
    may awaken.
  visibility: subject
- subject: character:tito
  object: character:quilla
  disposition: 66
  summary: Tito values Quilla's knowledge but carries private doubts about the lesson, the motive, or
    the price of succeeding.
  visibility: subject
history:
- id: connection-established
  title: The first difficult lesson
  summary: A demanding first lesson revealed both the student's potential and the teacher's exacting expectations.
    Neither could return to being a casual acquaintance afterward.
  occurred_at: When their connection took shape
  visibility: public
  stage_after: forming_mentor
  perspective_changes:
  - subject: character:quilla
    object: character:tito
    after: 56
    summary_after: Quilla believes Tito is worth the severity of the work and feels responsible for what
      the teaching may awaken.
    visibility: subject
  - subject: character:tito
    object: character:quilla
    after: 48
    summary_after: Tito values Quilla's knowledge but carries private doubts about the lesson, the motive,
      or the price of succeeding.
    visibility: subject
- id: present-balance
  title: The lesson began changing both of them
  summary: Quilla is passing on a craft that nearly died with her generation, and Tito is the only apprentice
    with the patience for it. What Tito does not yet know is that Quilla is also passing on the meanings
    encoded in the patterns — a ceremonial language the tribe believes is lost. She is deciding, lesson
    by lesson, whether he is ready to be told.
  occurred_at: During the present chapter
  visibility: participants
  stage_after: demanding_apprenticeship
  stage_before: forming_mentor
  relationship_summary_after: Quilla is passing on a craft that nearly died with her generation, and Tito
    is the only apprentice with the patience for it. What Tito does not yet know is that Quilla is also
    passing on the meanings encoded in the patterns — a ceremonial language the tribe believes is lost.
    She is deciding, lesson by lesson, whether he is ready to be told.
  perspective_changes:
  - subject: character:quilla
    object: character:tito
    before: 56
    after: 78
    summary_after: Quilla believes Tito is worth the severity of the work and feels responsible for what
      the teaching may awaken.
    visibility: subject
  - subject: character:tito
    object: character:quilla
    before: 48
    after: 66
    summary_after: Tito values Quilla's knowledge but carries private doubts about the lesson, the motive,
      or the price of succeeding.
    visibility: subject
visual:
  prompt: ''
image:
  url: ''
  focalPoint:
    x: 0.5
    y: 0.1
  seed: null
---
