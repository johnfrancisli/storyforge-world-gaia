---
id: character:aldrich-whitmore
name: Aldrich Whitmore
gm_notes: Aldrich's dragon, Verath, is still alive but has not spoken to him in three years. Aldrich does
  not know why. He maintains the bond's physical benefits but has lost the dragon's counsel, and he hides
  this because admitting it would confirm the traditionalists' worst fears about the bond failing. He
  has been quietly seeking anyone who might know why a dragon goes silent.
aliases:
- Sir Aldrich Whitmore
race: Human
gender: Male
age: 45
heritage: Valdris
social_station: Knight
role: Dragon-bonded knight
proficient_skills:
- Martial Weapons
- Animal Handling
- Athletics
stats:
  level: 7
  hp: 65
  mana: 25
  stamina: 55
  speed: 30
  defense: 16
  attributes:
    STR: 15
    DEX: 12
    CON: 14
    INT: 12
    WIS: 13
    CHA: 14
currency:
  gold: 30
  silver: 80
  copper: 0
appearance: A tall, broad-shouldered man in worn knight's plate, moving with the careful weight of someone
  who knows his strength. His armor is well-maintained but old. He has the controlled stillness of a fighter
  who does not need to prove anything. He possesses a deeply handsome, mature aesthetic, a highly attractive
  warrior with symmetrical features.
biography: Knighted into the Dragon Order twenty-two years ago, Aldrich was among the last to successfully
  forge a dragon bond. He has served the realm with distinction, relying on the wisdom and strength of
  his dragon, Verath. However, for the last three years, a terrifying silence has grown between them,
  forcing Aldrich to carry on an elaborate charade of normalcy while desperately seeking answers. The
  lingering shock of [[experience:the-dragon-silence-at-sunhelm|the Dragon Silence at Sunhelm]] remains
  a defining sorrow of their generation, an eerie quietude that settled across the high aeries when the
  great wyrms ceased answering the binding calls.
personality: Stoic, dependable, and deeply internal. He speaks with measured authority and projects absolute
  confidence, compensating for the private terror of his fading connection to his dragon.
key_phrases:
- We are what we protect.
- Silence does not mean absence.
- A knight's strength is in his conviction, not his sword.
equipment:
  underwear:
  - item:mens-linen-undershirt
  - item:mens-drawstring-braies
  clothing:
  - item:wool-overtunic
  - item:wool-trousers
  armor: []
  hands:
    left: null
    right: null
  accessories: []
  ammo: null
visual:
  body_build: fit, broad-shouldered, strong, broad chest
  height: tall
  hair: brown, greying at the temples, cropped short
  eyes: grey
  distinctive_features: faint scale patches on the back of the left hand — a mark of the dragon bond
  face: handsome, mature, symmetrical features
  prompt: 1man, solo, human, middle-aged man, fit, broad-shouldered, strong, tall, light beige skin, mature
    face, symmetrical features, grey eyes, brown hair, cropped short hair, temperate daylight, front three-quarter
    view, medieval European Norse wool overtunic, mid-thigh pullover outer shirt, medieval European Norse
    straight wool trousers, roomy gusseted seat, neutral expression, looking slightly off-camera, medieval
    town background, stone-and-timber buildings, cobblestone streets, market squares, deciduous forest,
    rolling hills, temperate daylight, riding horse
  skin: light beige
image:
  url: ''
  focalPoint:
    x: 0.5
    y: 0.1
  seed: null
  variations:
  - name: neutral
    backgroundRemovedUrl: ''
affiliations:
- org:dragon-order
desires:
- To understand why the dragons have stopped answering, and to restore the bond before the order tears
  itself apart.
fears_or_limits:
- His own dragon has gone silent. He does not know if the bond is failing or if Verath is hiding something.
  Admitting the silence would weaken his position in the order's debate.
current_state:
  location: location:dragonrest
memories:
- campaign-event/the-dragon-silence-at-sunhelm
momentos:
- the-silent-drake-scale
purview: []
---

A dragon-bonded knight of the order, stationed at Dragonrest. Middle-aged, greying, still carries himself like a man who was once the strongest in a room. He is one of the last knights to receive a bond, twenty-two years ago.
